...
typedef struct {
  ...
  int   devtype;
  struct {
    int   maxbuffers;       //!< Parameter maxbuffers
...
int operation_blit(count_handle * handle, sv_fifo_buffer * pbuffer)
{
    ...
    ** buffer (onboard memory) is split into fields!
    */
...
int operation_render(count_handle * handle, sv_fifo_buffer * pbuffer)
{
    ...
    // Toggle alpha value between 0 and 255.
    handle->alpha += handle->alpha_inc;
    if((handle->alpha < 0) || (handle->alpha > 255)) {
      handle->alpha_inc = -handle->alpha_inc;
      handle->alpha += handle->alpha_inc;
    }
    init_colors(handle, &handle->storage, NULL, NULL);
...
int count_paint_thread(void * voidhandle)
{
    ...
    int               dropped_count = 0;
    int               fifoflags     = 0;
    int               fiforunning   = FALSE;
    char card[128]    = "";

    memset(&finfo, 0, sizeof(sv_fifo_info));
     ...
     */
    if(handle->setup.maxbuffers) {
      maxbuffers = handle->setup.maxbuffers;
    } else {
      if(handle->setup.fieldbased) {
        maxbuffers = 4 /*Fields*/;
      } else {
      ...
      if(handle->devtype == SV_DEVTYPE_HYDRA) {
        if(handle->setup.fieldbased) {
          maxbuffers += 12 /*Fields*/;
        } else {
          maxbuffers += 6 /*Frames*/;
        }
      }
    ...
    */
    res = sv_fifo_init(sv, &poutput, FALSE, FALSE, TRUE, fifoflags | SV_FIFO_FLAG_VIDEOONLY, 0);
    ...
    if(res != SV_OK)  {
      ...
      for(k = 0; k < COUNTER_MAXBUFFERS; k++) {
        if((handle->buffers.table[k].buffer_org = malloc(handle->buffers.framesize + 0xff)) == NULL) {
          running = FALSE;
        } else {
          handle->buffers.table[k].buffer = (void*)(((uintptr)handle->buffers.table[k].buffer_org + 0xff) & ~0xff);
        }
    ...
    while(running && handle->running) {
        do {
          ...
          if((finfo.nbuffers - finfo.availbuffers) > maxbuffers) {
        ...
        } while(handle->running && ((finfo.nbuffers - finfo.availbuffers) > maxbuffers));

        /**
        //  Fetch the buffer to the next frame to be filled in.
        //
        */
        res = sv_fifo_getbuffer(sv, poutput, &pbuffer, NULL, fifoflags);
        if(res != SV_OK)  {
        ...
        */
        ...
        pbuffer->timecode.vitc_tc2       = hex2tc(handle->loop);
        ...
        */
        res = sv_fifo_putbuffer(sv, poutput, pbuffer, &infodst);
        if(res != SV_OK)  {
        ...
        if( handle->verbose || finfo.dropped>dropped_count ) {
          printf("o: avail %02d drop %02d  nbuf %02d  tick %06d  delay %d\n", finfo.availbuffers, finfo.dropped, finfo.nbuffers, finfo.tick, infodst.when - finfo.tick);	/**/
            putchar('.');
            fflush(stdout);
            dropped_count = finfo.dropped;
        }

        if(!fiforunning && ((finfo.nbuffers-finfo.availbuffers) >= 1) ) {
          /*
...
void convertbuffer(sv_storageinfo * pstorage, unsigned char * q, int inputsize, unsigned char * p, int * outputsize)
{
  ...
  if(pstorage->nbits == 12) {
    switch(pstorage->nbittype) {
    case SV_NBITTYPE_12B:
      x = inputsize;
      while(3 * x % 2) {
        x += inputsize;
      }
      inputsize = x;
      for(x = 0; x < inputsize; x+=2) {
        p[0] = q[0];
        p[1] = q[1]>>4;
        p[2] = q[1]<<4;
        p+=3;
      ...
      break;
    default:
      printf("Error unknown 12 bit mode\n");
      exit(-1);
    }
  } else if(pstorage->nbits == 10) {
    x = inputsize;
    while(4 * x % 3) {
      x += inputsize;
    }
  ...
  } else if(pstorage->nbits == 8) {
    memcpy(p, q, inputsize);
  } else {
    printf("Error unknown bit mode\n");
    exit(-1);
  }
...
int count_init(count_handle * handle) 
{
    ...
    res = sv_query(sv, SV_QUERY_DEVTYPE, 0, &handle->devtype);
    if(res != SV_OK) {
      printf("count_init: SV_QUERY_DEVTYPE failed = %d '%s'\n", res, sv_geterrortext(res));
      sv_close(sv);
      return FALSE;
    }
    ...
    if(!init_colors(handle, &storage, &psize, &xzoom)) {
      return FALSE;
    }
...
{ 
  ...
  while((argc > 1) && (argv[1][0] == '-')) {
    ...
    case 'l':
      ...
      break;
    case 'm':
      handle->setup.maxbuffers = atoi(&argv[1][3]);
      break;
    case 'v':
      handle->verbose = TRUE;
      break;
