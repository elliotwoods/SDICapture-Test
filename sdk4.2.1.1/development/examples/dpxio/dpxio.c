...
//
//  This chapter shows the use of the FIFO API (see chapter \ref fifoapi) to display and record video from/to DPX
//  files and audio from/to an AIFF file.
//  @{
*/
...
int dpxio_ancstream_layout(int videomode, int field, int * hancsize, int * vancsize, int * linebegin, int * lineend)
{
...
int dpxio_dump_ancstream(unsigned short * anc, int size, int videomode, int field)
{
  int hancsize;
  int vancsize;
  int linebegin;
  ...
  int position;
  int value;
  unsigned short * start = anc;
  int remain = 0;
  int area;
  int areasize;

  if(!dpxio_ancstream_layout(videomode, field, &hancsize, &vancsize, &linebegin, &lineend)) {
  ...
  for(line = linebegin; line <= lineend; line++) {
    for(area = 0; area < 2; area++) {
      ...
      for(position = 0; position < areasize; position++, anc++) {
        value = (*anc) & 0x3ff;
        if(remain > 0) {
          remain--;
          printf("%03x ", value);
        } else if(((anc[0]&0x3ff)==0x0) && ((anc[1]&0x3ff)==0x3ff) && ((anc[2]&0x3ff)==0x3ff)) {
          remain = (anc[5] & 0xff) + 4; // datacount
          printf("\n000 3ff 3ff: ");
          anc+=2;
          position+=2;
        } else if((anc[0]&0x3ff) != ((position & 1) ? 0x040 : 0x200)) {
          printf("\nERROR: %d> %03x != %03x (invalid stream)", position, anc[0], (position & 1) ? 0x040 : 0x200);
        }
      }
...
void dpxio_generate_ancpacket(unsigned short * anc_packet, int size, int did, int sdid, int line, int field, int packet)
{
...
#define DCPOSITION     5
  ...
  if( size >= HEADERSIZE + 2 ) {
    ...
    anc_packet[data_pos++] = ancparity[((packet<<4)&0xff)|(field&0x0f)]; //Packet Number and Field
    dc+=2;
  }
...
int dpxio_generate_ancstream(unsigned short * anc, int size, int npackets, int videomode, int field)
{
  // buffer for one
  unsigned short anc_packet[ HEADERSIZE + USERDATASIZE + CHECKSUMSIZE ];
  int hancsize;
  int vancsize;
  int linebegin;
  int lineend;
  ...
  // Fill complete buffer to be a valid SDI stream.
  while(((anc - start) * 2) < size) {
    *(anc++) = 0x200;
    *(anc++) = 0x040;
  }
  anc = start;
  ...
  for(line = linebegin; line <= lineend; line++) {
    ...
    for(packet = 0; packet < npackets; packet++) {
      // Generate packet
      dpxio_generate_ancpacket(anc_packet, arraysize(anc_packet), 0x5f, 0xfa, line, field, packet+1);

      // Fill packet(s) into VANC
      for(pos = 0; pos < arraysize(anc_packet); pos++) {
        *(anc++) = anc_packet[pos];
      }
    }

    // Skip remaining part of VANC
    anc += vancsize - ((int)arraysize(anc_packet)*npackets);
  }
...
int dpxio_exec(dpxio_handle * dpxio, int framecount)
{
  ...
  int                   vsize[DPXIO_MAXBUFFER] = { 0 };
  unsigned char         lut[6][32768];

  unsigned int vtimecode;
  unsigned int atimecode[16];
  ...
  int   fifoflags = 0;
  int   i,j;
  sv_fifo_linkencrypt_info le_info = { 0 };
  sv_fifo_configinfo config = {0};
  unsigned short * anc     = 0;
  unsigned short * anc_org = 0;
  unsigned short * anc_field[2] = { 0 };
  int     syncstate = 0;
  int     timeout;
  ...
  */
  ...
  if(dpxio->config.stereomode != VIDEO_STEREO_NONE) {
    flagbase  |= SV_FIFO_FLAG_STEREO; /* This is needed to enable to correct memory allocation */
    fifoflags |= SV_FIFO_FLAG_STEREO;
  }
  ...
  */
  ...
  if(dpxio->config.ancstream && config.ancbuffersize) {
    ...
    anc = (unsigned short *)(((uintptr)anc_org + (config.dmaalignment-1)) & ~(config.dmaalignment-1));
  ...
  if(dpxio->config.ancstream) {
    ...
    res = sv_fifo_anclayout(dpxio->sv, pfifo, anclayout, sizeof(anclayout), &required);
  ...
  //  <br>As a next step we allocate DMA buffers because it is good practice to page-align the buffers. This is not really needed, but
  //  due to the large sizes of video there will be no scatter/gather block for the DMA to start
  //  the transfer. All DMA buffers must at least be at aligned to the size returned from the driver
  //  for the board's minimum DMA alignment. You can query the minimum alignment required for your DVS video board with the define \c #SV_QUERY_DMAALIGNMENT.
  //
  */
  if(running) {
  ...
  for(frame = 0; (running && (frame < framecount)); frame++) {
    ...
    */
    ...
    if(dpxio->config.le_nkeys) {
      int link;
      int key = (dpxio->config.le_nkeys > 1) ? ((frame / 100) & 1) : 0;

      for(link = 0; link < 2; link++) {
        le_info.link[link].activate = TRUE;

        le_info.link[link].metadata.linenr = 0; // use default line

        le_info.link[link].metadata.current_key = dpxio->config.le[key].keyid;
        le_info.link[link].metadata.next_key    = dpxio->config.le[!key].keyid;

        for(i = 0; i < 16; i++) {
          le_info.link[link].aes.key[i]  = dpxio->config.le[key].key[i];
        }
        for(i = 0; i < 8; i++) {
          le_info.link[link].aes.attr[i] = dpxio->config.le[key].attr[i];
        }
      }

      res = sv_fifo_linkencrypt(dpxio->sv, pfifo, pbuffer, &le_info);
      if(res != SV_OK)  {
        printf("ERROR: sv_fifo_linkencrypt(sv) failed = %d '%s'\n", res, sv_geterrortext(res));
        running = FALSE;
      }
    }

    if(dpxio->config.ancstream) {
      pbuffer->anc[0].addr += (uintptr)anc;
      pbuffer->anc[1].addr += (uintptr)anc;

      // Remember these field buffer addresses.
      anc_field[0] = (unsigned short *)pbuffer->anc[0].addr;
      anc_field[1] = (unsigned short *)pbuffer->anc[1].addr;
    }
    ...
    */
    if(!dpxio->binput) {
      ...
      if(dpxio->config.anccount) {
        ...
        do {
          delta = line - dpxio->config.ancfill;
          ancbuffer.did       = 0xc0 + delta;
          ancbuffer.sdid      = frame & 0xff;
          ancbuffer.datasize  = (4 + delta) & 0xff;
          ancbuffer.vanc      = FALSE;
          ancbuffer.linenr    = line;
          for(i = 0; i < ancbuffer.datasize; i++) {
            ancbuffer.data[i] = value++;
          }
          res = sv_fifo_anc(dpxio->sv, pfifo, pbuffer, &ancbuffer);
          // if possible, embed both types of anc packets
          if(line <= dpxio->hw.maxvancline) {
            ancbuffer.vanc = TRUE;
            res = sv_fifo_anc(dpxio->sv, pfifo, pbuffer, &ancbuffer);
          } 
      ...
      if(dpxio->config.ancstream) {
        if(res == SV_OK) {
          res = dpxio_generate_ancstream(anc_field[0], config.ancbuffersize, dpxio->config.ancstream, dpxio->hw.videomode, 0);
          if(res != SV_OK) {
            printf("ERROR: dpxio_generate_ancstream(field0) failed\n");
            running = FALSE;
          }
        }
        if(res == SV_OK) {
          res = dpxio_generate_ancstream(anc_field[1], config.ancbuffersize, dpxio->config.ancstream, dpxio->hw.videomode, 1);
          if(res != SV_OK) {
            printf("ERROR: dpxio_generate_ancstream(field1) failed\n");
            running = FALSE;
          }
        }
        if(dpxio->verbose) {
          dpxio_dump_ancstream(anc_field[0], config.ancbuffersize, dpxio->hw.videomode, 0);
          dpxio_dump_ancstream(anc_field[1], config.ancbuffersize, dpxio->hw.videomode, 1);
        }
      }
    }
    ...
    if(!dpxio->binput) {
      ...
      if(dpxio->bvideo && running) {
        if(dpxio->config.stereomode != VIDEO_STEREO_NONE) {
          int fileindex[2] = {0};
          int frameindex[2] = {0};
          if(dpxio->config.stereomode == VIDEO_STEREO_DUALSTREAM) {
            fileindex[0] = 0;
            fileindex[1] = 1;
            frameindex[0] = frame;
            frameindex[1] = frame;
          } else if(dpxio->config.stereomode == VIDEO_STEREO_INTERLEAVED) {
            fileindex[0] = 0;
            fileindex[1] = 0;
            frameindex[0] = 2 * frame + 0;
            frameindex[1] = 2 * frame + 1;
          }
          vsize[0] = dpxio_video_readframe(dpxio, fileindex[0], frameindex[0], pbuffer, videobuffer[0], DPXIO_BUFFERVIDEOSIZE, &offset, &xsize, &ysize, &dpxtype, &nbits, &lineoffset);
          if(!vsize[0]) {
            printf("ERROR: dpxio_video_readframe(sv) 3D left eye failed\n");
            running = FALSE;
          }
          vsize[1] = dpxio_video_readframe(dpxio, fileindex[1], frameindex[1], pbuffer, videobuffer[1], DPXIO_BUFFERVIDEOSIZE, &offset, &xsize, &ysize, &dpxtype, &nbits, &lineoffset);
          if(!vsize[1]) {
            printf("ERROR: dpxio_video_readframe(sv) 3D right eye failed\n");
            running = FALSE;
          }
        } else {
          vsize[0] = dpxio_video_readframe(dpxio, 0, frame, pbuffer, videobuffer[0], DPXIO_BUFFERVIDEOSIZE, &offset, &xsize, &ysize, &dpxtype, &nbits, &lineoffset);
          if(!vsize[0]) {
        ...
        if(frame < DPXIO_MAX_TRACE) {
          dpxio->trace[frame].framesize = vsize[0];
        }
      }
      if((frame == 0) && !dpxio->io.dryrun && dpxio->bvideo) {
        ...
        //  To see an example for this it is suggested to take a look at the <code><i>cmodetst</i></code> program.
        //
      ...
      vsize[0] = dpxio_framesize(dpxio, xsize, ysize, dpxtype, nbits, &offset, &padding);
    }

    if(dpxio->bvideo) {
      pbuffer->video[field].addr =  videobuffer[0] + (offset & ~0xfff);
      pbuffer->video[field].size = (vsize[0] - (offset & ~0xfff) + 0xff) & ~0xff;
      if(dpxio->config.stereomode != VIDEO_STEREO_NONE) {
        pbuffer->stereo.video[field].addr =  videobuffer[1] + (offset & ~0xfff);
        pbuffer->stereo.video[field].size = (vsize[1] - (offset & ~0xfff) + 0xff) & ~0xff;
      }
    } else {
    ...
    */
    ...
    if(running) {
      ...
      if(dpxio->binput) {
        if(dpxio->config.ancstream) {
          if(res == SV_OK) {
            res = dpxio_dump_ancstream(anc_field[0], config.ancbuffersize, dpxio->hw.videomode, 0);
            if(res != SV_OK) {
              printf("ERROR: dpxio_dump_ancstream(field0) failed\n");
              running = FALSE;
            }
          }
          if(res == SV_OK) {
            res = dpxio_dump_ancstream(anc_field[1], config.ancbuffersize, dpxio->hw.videomode, 1);
            if(res != SV_OK) {
              printf("ERROR: dpxio_dump_ancstream(field1) failed\n");
              running = FALSE;
            }
          }
        }
      ...
      vtimecode = 0;
      memset(&atimecode[0], 0, sizeof(atimecode));
      memset(&aoffset[0],   0, sizeof(aoffset));
      if(dpxio->config.verbosetc || (dpxio->config.verifytimecode && ((frame == 0) || (dpxio->config.verifytimecode == 2)))) {
        vtimecode = dpxio_dpx_timecode((unsigned char *)pbuffer->video[0].addr, dpxtype, xsize);
        if(dpxio->baudio) {
          dpxio_audio_timecode((unsigned char *)audiobuffer[0], pbuffer->audio[0].size / 4 / (2 * dpxio->hw.audiochannels), 2 * dpxio->hw.audiochannels, atimecode, aoffset);
        }
      }
      ...
      if((dpxio->config.verifytimecode && ((frame == 0) || (dpxio->config.verifytimecode == 2)))) {
        ...
        //}
        if(tcerror) {
          printf("ERROR: ltc:%08x dvitc:%08x vtr:%08x video:%08x audio:%08x/%d - %08x\n", pbuffer->timecode.ltc_tc, pbuffer->anctimecode.dvitc_tc[0], pbuffer->timecode.vtr_tc, vtimecode, atimecode[0], aoffset[0], xtimecode);
        }
      ...
      if(dpxio->config.verbosetc) {
        printf("      %3d ltc:%08x dvitc:%08x vtr:%08x video:%08x audio:%08x/%d %08x/%d\n", frame, pbuffer->timecode.ltc_tc, pbuffer->anctimecode.dvitc_tc[0], pbuffer->timecode.vtr_tc, vtimecode, atimecode[0], aoffset[0], atimecode[1], aoffset[1]);
      }
      ...
      if(dpxio->binput) {
        ...
        if(dpxio->bvideo && running) {
          ...
          dpxio_video_writeframe(dpxio, 0, frame, pbuffer, videobuffer[0], vsize[0], xsize, ysize, dpxtype, nbits, offset, padding, timecode);
        }
...
int main(int argc, char ** argv)
{
  ...
  while(!error && (argc > 2) && (argv[1][0] == '-')) {
    switch(argv[1][1]) {
    case '3':
      if(!strncmp(&argv[1][1], "3dsingle", 8)) {
        dpxio->config.stereomode = VIDEO_STEREO_INTERLEAVED;
      } else if(!strncmp(&argv[1][1], "3ddual", 6)) {
        dpxio->config.stereomode = VIDEO_STEREO_DUALSTREAM;
      } else {
        error = TRUE;
      }
      break;
    ...
    case 'b':
      ...
      break;
    case '#':
      dpxio->config.le[0].keyid = atoi(&argv[1][3]);
      dpxio->config.le[1].keyid = dpxio->config.le[0].keyid + 1;
      break;
    case 'c':
      if(dpxio->config.le_nkeys < arraysize(dpxio->config.le)) {
        char str_key[32+1] = "00000000000000000000000000000000";
        char str_attr[16+1] = "0000000000000000";
        char tmp[3];
        int i;

        if(strlen(&argv[1][3]) != 32+1+16) {
          printf("ERROR: Invalid LE key parameter '%s'\n", argv[1]);
          error = TRUE;
        } else {
          strncpy(str_key,  &argv[1][3]     , 32);
          strncpy(str_attr, &argv[1][3+32+1], 16);
        }
        if(!error) {
          for(i = 0; i < 16; i++) {
            tmp[0] = str_key[i*2];
            tmp[1] = str_key[i*2+1];
            tmp[2] = '\0';
            dpxio->config.le[dpxio->config.le_nkeys].key[i] = strtoul(tmp, NULL, 16);
          }
          for(i = 0; i < 8; i++) {
            tmp[0] = str_attr[i*2];
            tmp[1] = str_attr[i*2+1];
            tmp[2] = '\0';
            dpxio->config.le[dpxio->config.le_nkeys].attr[i] = strtoul(tmp, NULL, 16);
          }
        }
        dpxio->config.le_nkeys++;
      }
      break;
    case 'd':
    ...
    case 'v':
      ...
      case ' ':
        ...
        break;
      case 'd':
        dpxio->config.dumpjp2files = TRUE;
        break;
      case 't':
  ...
  if(!error) {
    char * pvideoname  = "dummy";
    char * pstereoname = "dummy";
    char * paudioname  = "dummy";
    ...
    if(argc >= 5) {
      ...
      pvideoname = argv[2]; argv++; argc--;
      dpxio->bvideo = TRUE;

      if(dpxio->config.stereomode == VIDEO_STEREO_DUALSTREAM) {
        if(argc >= 5) {
          pstereoname = argv[2]; argv++; argc--;
        } else {
          printf("ERROR: Wrong number of parameters for 3D dual stream\n");
          noprint = error = TRUE;
        }
      }

      if(argc >= 5) {
        paudioname = argv[2]; argv++; argc--;
        dpxio->baudio = TRUE;
      }

      if(dpxio->bvideo) {
        strcpy(dpxio->videoio[0].filename, pvideoname);
        if (!strcmp(dpxio->videoio[0].filename, "dummy")) {
          if(dpxio->config.verbosetc) {
            printf("ERROR: Illegal combination of special filename 'dummy' with option '-vu'\n");
            error = TRUE;
          } else {
            dpxio->bvideo = FALSE;
          }
        ...
        if(dpxio->config.stereomode == VIDEO_STEREO_DUALSTREAM) {
          strcpy(dpxio->videoio[1].filename, pstereoname);
          if (!strcmp(dpxio->videoio[1].filename, "dummy")) {
            if(dpxio->config.verbosetc) {
              printf("ERROR: Illegal combination of special filename 'dummy' with option '-vu'\n");
              error = TRUE;
            } else {
      ...
      if(dpxio->baudio) {
        strcpy(dpxio->audioio.filename, paudioname);
      ...
      dpxio->videoio[0].framenr = atoi(argv[2]);
  ...
  for(i = 1; i < arraysize(dpxio->videoio); i++) {
    dpxio->videoio[i].framenr = dpxio->videoio[0].framenr;
  }
  ...
  if(!error) {
    ...
    if(!error) {
      ...
      res = sv_jack_option_get(dpxio->sv, dpxio->binput ? 1 : 0, SV_OPTION_VIDEOMODE, &dpxio->hw.videomode);
      if(res != SV_OK) {
        printf("ERROR: sv_jack_option_get(SV_OPTION_VIDEOMODE) failed = %d\n", res);
        noprint = error = TRUE;
      }

      res = sv_jack_query(dpxio->sv, dpxio->binput ? 1 : 0, SV_QUERY_AUDIOCHANNELS, 0, &dpxio->hw.audiochannels);
      if(res != SV_OK) {
        printf("ERROR: sv_jack_query(SV_QUERY_AUDIOCHANNELS) failed = %d\n", res);
        noprint = error = TRUE;
      } 
      ...
      res = sv_jack_query(dpxio->sv, dpxio->binput ? 1 : 0, SV_QUERY_ANC_MAXVANCLINENR, 0, &dpxio->hw.maxvancline);
      if(res != SV_OK) {
        printf("ERROR: sv_jack_query(SV_QUERY_ANC_MAXVANCLINENR) failed = %d\n", res);
        noprint = error = TRUE;
      } 

      if(dpxio->binput) {
        if(((tmp & (SV_MODE_NBIT_MASK | SV_MODE_COLOR_MASK)) != (SV_MODE_NBIT_10BDPX | SV_MODE_COLOR_RGB_RGB)) &&
           ((tmp & (SV_MODE_NBIT_MASK | SV_MODE_COLOR_MASK)) != (SV_MODE_NBIT_10BDPX | SV_MODE_COLOR_YUV422)) &&
           ((tmp & (SV_MODE_NBIT_MASK | SV_MODE_COLOR_MASK)) != (SV_MODE_NBIT_8B     | SV_MODE_COLOR_YUV422))
           ){
          printf("ERROR: Not running in RGB/10BDPX or YUV422/10BDPX or YUV422/8B mode\n");
          noprint = error = TRUE;
        }
      }
    ...
    do {
      if(dpxio->bvideo) {
        if(!error) {
          if(dpxio->binput) {
            if(!dpxio_setvideoformat(dpxio, dpxio->videoio[0].filename)) {
              printf("ERROR: Could not set video file format\n");
              noprint = error = TRUE;
            }
          } else {
            if(dpxio->config.stereomode == VIDEO_STEREO_DUALSTREAM) {
              if(!dpxio_verifyformat(dpxio, dpxio->videoio[0].filename, dpxio->videoio[0].framenr)) {
                printf("ERROR: Could not determine 3D left video file format\n");
                noprint = error = TRUE;
              } else if(!dpxio_verifyformat(dpxio, dpxio->videoio[1].filename, dpxio->videoio[1].framenr)) {
                printf("ERROR: Could not determine 3D right video stereo file format\n");
                noprint = error = TRUE;
              }
            } else if(dpxio->config.stereomode == VIDEO_STEREO_INTERLEAVED) {
              if(!dpxio_verifyformat(dpxio, dpxio->videoio[0].filename, dpxio->videoio[0].framenr)) {
                printf("ERROR: Could not determine interleaved 3D video file format\n");
                noprint = error = TRUE;
              } 
            } else {
              if(!dpxio_verifyformat(dpxio, dpxio->videoio[0].filename, dpxio->videoio[0].framenr)) {
        ...
        if(!error) {
          if(!dpxio_video_opensequence(dpxio, 0, dpxio->videoio[0].filename, dpxio->videoio[0].framenr, dpxio->videoio[0].framenr + framecount)) {
            printf("ERROR: Error opening video files");
            noprint = error = TRUE;
          }
          if(dpxio->config.stereomode == VIDEO_STEREO_DUALSTREAM) {
            if(!dpxio_video_opensequence(dpxio, 1, dpxio->videoio[1].filename, dpxio->videoio[1].framenr, dpxio->videoio[1].framenr + framecount)) {
              printf("ERROR: Error opening video files");
              noprint = error = TRUE;
            }
          }
      ...
      if(!error) {
        ...
        if(dpxio->bvideo) {
          if(!dpxio_video_closesequence(dpxio, 0)) {
            printf("ERROR: Error closing video files");
            noprint = error = TRUE;
          }
          if(dpxio->config.stereomode == VIDEO_STEREO_DUALSTREAM) {
            if(!dpxio_video_closesequence(dpxio, 1)) {
              printf("ERROR: Error closing video files (stereo)");
              noprint = error = TRUE;
            }
          }
        }
  ...
  if(error && !noprint) { 
    ...
    printf("         -c=key      Link encryption (format <key>:<attr>)\n");
    printf("         -3dsingle   Enable stereo output, L-R interleaved sequence.\n");
    printf("         -3ddual     Enable stereo output, dual stream sequences.\n");
    printf("         -d          Dry-run\n");
    ...
    printf("         -v          Verbose.\n");
    printf("         -vd         Dump jp2 files from mxf files.\n");
    printf("         -vt         Display datarate.\n");
    printf("         -vu         Verbose timing / LTC / VTR. (record)\n");
    printf("                     NOTE! Don't use option '-vu'with special filename 'dummy'!\n");
