...
/************************** MXF FILE ***************************/
static int dpxio_mxf_open(dpxio_handle * dpxio, dpxio_fileio * pfileio, char * filename)
{
  if(!dpxio_mxf_opensequence(dpxio, pfileio, TRUE, filename, 0, 0)) {
...
static int dpxio_aiff_create(dpxio_handle * dpxio, char * filename, int frequency, int channels, int nbits)
{
  ...
  handle = file_open(filename, O_RDWR|O_BINARY|O_CREAT, 0666, TRUE);
  if(handle == NULL) {
...
int dpxio_audio_open(dpxio_handle * dpxio, char * filename, int hwchannels, int * pchannels, int * pfrequency)
{
  ...
  case AUDIO_FORMAT_WAVE:
    ...
    break;
  case AUDIO_FORMAT_MXF:
    format = dpxio_mxf_open(dpxio, &dpxio->audioio, filename);
    break;
  default:
    format = AUDIO_FORMAT_INVALID;
  }
...
static int dpxio_audio_do_read(dpxio_handle * dpxio, dpxio_fileio * paudioio, int framenr, sv_fifo_buffer * pbuffer, void * buffer, int * size)
{
  int nsamples;
  int datasize;
  int res;

  res = dpxio_mxf_readaudio(dpxio, paudioio, framenr, pbuffer, dpxio->audio.filebuffer, dpxio->audio.filebuffersize, &datasize);
...
{
  dpxio_fileio * pfileio = &dpxio->audioio;
  int datacount;
  int res = TRUE;

  if(dpxio->audio.format == AUDIO_FORMAT_MXF) {
    if(dpxio->audioio.decrypt) {
      res = dpxio_mxf_readaudio(dpxio, pfileio, framenr, pbuffer, buffer, dpxio->audio.filebuffersize, NULL);
    } else {
      ...
      while(res && nsamples > 0) {
        res = dpxio_audio_do_read(dpxio, pfileio, framenr, pbuffer, src, &size);
    ...
    if(size > dpxio->audio.filebuffersize) {
      return FALSE;
    } 
    
    res = file_read(dpxio->audioio.handle, dpxio->audio.filebuffer, size);
