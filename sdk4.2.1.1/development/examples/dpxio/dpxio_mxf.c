...
int dpxio_mxf_opensequence(dpxio_handle * dpxio, dpxio_fileio * pfileio, int baudio, char * filename, int firstframe, int lastframe)
{
  ...
  if(baudio) {
    ...
    pfileio->handle   = handle;
    pfileio->position = tableoffset;
    pfileio->xsize  = 2048;
    pfileio->ysize  = 1080;
  }
...
int dpxio_mxf_closesequence(dpxio_handle * dpxio, dpxio_fileio * pfileio, int baudio)
{
  if(baudio) {
    file_close(dpxio->audioio.handle); dpxio->audioio.handle = NULL;
  } else {
    file_close(pfileio->handle); pfileio->handle = NULL;
  }
...
int dpxio_mxf_readframe(dpxio_handle * dpxio, dpxio_fileio * pfileio, int framenr, sv_fifo_buffer * pbuffer, char * buffer, int buffersize, int * offset, int * xsize, int * ysize, int * dpxtype, int * nbits)
{
  uint64 result;
  int size;
  int i;
  int preread = pfileio->decrypt ? 128 : 20;
  int pos = 0;
  int found;
  int totalsize;
  int dummy;
  ...
  if(dpxio->io.dryrun) {
    ...
    result = file_lseek(pfileio->handle, pfileio->position, SEEK_SET);
    if(result) {
      if(file_read(pfileio->handle, buffer, preread) != preread) {
        return 0;
      }
      ...
      if(pfileio->decrypt) {
        ...
        result = file_lseek(pfileio->handle, pfileio->position + pos, SEEK_SET);
      } else {
        payload = size;
      }
      ...
      if(dpxio->verbose) {
#ifdef WIN32
        printf("read frame(%dx%d) %d @ %16I64x %d/%08x\n", *xsize, *ysize, framenr, pfileio->position, payload, payload);
#else
        printf("read frame(%dx%d) %d @ %llx %d/%08x\n", *xsize, *ysize, framenr, pfileio->position, payload, payload);
#endif
      }
      file_read(pfileio->handle, buffer, payload);

      if(dpxio->config.dumpjp2files) {
        char tmp[43];
        sprintf(tmp, "file%04d.jp2", framenr);
        {
          FILE * fp = fopen(tmp, "wb");
          if(fp) {
            fwrite(buffer, 1, payload, fp);
            fclose(fp);
          }
        }
      }
...
int dpxio_mxf_writeframe(dpxio_handle * dpxio, dpxio_fileio * pfileio, int framenr, sv_fifo_buffer * pbuffer, char * buffer, int buffersize, int xsize, int ysize, int dpxtype, int nbits, int offset, int padding, int timecode)
{
#if 0
...
int dpxio_mxf_readaudio(dpxio_handle * dpxio, dpxio_fileio * pfileio, int framenr, sv_fifo_buffer * pbuffer, char * buffer, int buffersize, int * datasize)
{
  uint64 result;
  int size;
  int i;
  ...
  if(dpxio->io.dryrun) {
    ...
    if(dpxio->audioio.decrypt) {
      ...
      pbuffer->encryption_audio.keyid = dpxio->config.key_base + (pfileio->decrypt ? 1 : 0);
