...
int dpxformat_getstoragemode(int dpxtype, int nbits)
{
  ...
  case 12:
    if(bigendian) {
      ...
      storagemode  |= SV_MODE_NBIT_12BDPXLE;
    }
    break;
  case 16:
    if(bigendian) {
      ...
      storagemode  |= SV_MODE_NBIT_16BLE;
    }
    break;
  default:
    if(dpxv2format) {
      if(bigendian) {
        if((dpxtype & 0xff) > 6) {
          ...
          storagemode |= SV_MODE_NBIT_10BRABE;
        }
      } else {
        storagemode |= SV_MODE_NBIT_10BLALEV2;
      }
    } else {
      if(bigendian || ((dpxtype & 0xff) <= 6)) {
  ...
  if(get32(&pheader[0], swap) == 0x53445058) {
    ...
    if(dpxtype) {
      *dpxtype  = pheader[800];
      ...
      if(!strcmp((char*)&pheader[160], "LIBFORMAT")) {
        *dpxtype |= 0x200;
      }
  
      if(pheader[800] == 0) {
        switch(get32(&pheader[20], swap)) {
        case 1: // BGGR
          *dpxtype |= 0x1400;
          break;
        case 2: // GBRG
