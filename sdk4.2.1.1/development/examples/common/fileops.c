...
static int win32_rw(void * file, int bwrite, char * buffer, int size)
{
  ...
  do {
    ...
    buffer   += size;
    nbytes   += result;
  } while(res && (result == (DWORD)size) && (restsize > 0));
...
static void * unix_open(char *filename, int mode, int createmode, int buffering)
{
  ...
  */
  ...
  return (void *)(uintptr)fd;
}

static void unix_close(void * file)
{
  close((int)(uintptr)file);
}
...
static int unix_read(void * file, char *buffer, int size, int * pcookie)
{
  ...
  return read((int)(uintptr)file, buffer, size);
}
...
static int unix_write(void * file, char *buffer, int size, int * pcookie)
{
  ...
  return write((int)(uintptr)file, buffer, size);
}
...
{
#ifdef linux
  return lseek64((int)(uintptr)file, position, mode);
#else
  return lseek((int)(uintptr)file, position, mode);
#endif
}
