...
int user_help(int argc, char ** argv)
{
  if(argc <= 1) {
    ...
    if(!strcmp(argv[1], "display") || !strcmp(argv[1], "record")) {
      ...
      printf("       svram mainoutput help\tSet main output (e.g. SDI or DVI)\n");
...
{ 
  ...
  if(argc > 1) {
	...
	/* FALLTHROUGH */
      ...
      ok = 1;
    } else if((argc >= 3) && (strcmp(argv[1], "debugoption") == 0)) {
      jpeg_debugoption(sv, argc-1, argv+1);
      ok = 1;
      ...
      ok = 1;
    } else if((argc >= 3) && !strcmp(argv[1], "audioaessource")) {
      jpeg_audioaessource(sv, argc-2, &argv[2]);
      ok = 1;
      ...
      ok = 1;
    } else if((argc >= 3) && !strcmp(argv[1], "hdmioutput")) {
      if(argc >= 4) {
        jpeg_hdmioutput(sv, argv[2], argv[3]);
      } else {
        jpeg_hdmioutput(sv, argv[2], NULL);
      }
      ok = 1;
