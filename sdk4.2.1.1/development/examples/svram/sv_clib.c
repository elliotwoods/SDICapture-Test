...
void jpeg_info_input(sv_handle * sv)
{
  ...
  res = sv_query(sv, SV_QUERY_SMPTE352, 1, &temp);
  if(res == SV_OK) {
    printf("Smpte352b           : %02x %02x %02x %02x\n", temp & 0xff, (temp >> 8) & 0xff, (temp >> 16) & 0xff, (temp >> 24) & 0xff);
  }
...
void jpeg_proxy(sv_handle * sv, int argc, char ** argv)
{
  ...
  if(argc > 0) {
    if(!strcmp(argv[0], "info")) {
      ...
      if(res == SV_OK) {
        printf("sv proxy option %s%s%s%s%s%s%s\n", 
          (tmp == 0)?"none":"", 
          ...
          (tmp & SV_PROXY_OPTION_LOWRESOLUTION)?"lowresolution ":"",
          (tmp & SV_PROXY_OPTION_640X480)?"640x480 ":"");
      }
    } else if((strcmp(argv[0], "?") == 0) || (strcmp(argv[0], "help") == 0)) {
      printf("sv proxy help\n");
      printf("sv proxy mode {PAL,NTSC}\tSet proxy videmode\n");
      printf("sv proxy sync {auto,internal,genlock}\n");
      printf("sv proxy option [ntscjapan,sdtvfull,freezefield,desktoponly,lowresolution,640x480,none]\n");
      ...
      for(tmp = 0, i = 1; i < argc; i++) {
        ...
        } else if(!strcmp(argv[i], "lowresolution")) {
          tmp |= SV_PROXY_OPTION_LOWRESOLUTION;
        } else if(!strcmp(argv[i], "640x480")) {
          tmp |= SV_PROXY_OPTION_640X480;
  ...
  if(argc >= 1) {
    if(!strcmp(argv[0], "info")) {
      ...
      if(res == SV_OK) {
        ...
        } else if(value == SV_AUDIOAESROUTING_8_0) {
          printf("svram audioaesrouting 8 0\n");
        } else if(value == SV_AUDIOAESROUTING_0_8) {
          printf("svram audioaesrouting 0 8\n");
        } else {
      ...
      } else if((channels_a == 8) && (channels_b == 0)) {
        res = sv_option_set(sv, SV_OPTION_AUDIOAESROUTING, SV_AUDIOAESROUTING_8_0);
      } else if((channels_a == 0) && (channels_b == 8)) {
        res = sv_option_set(sv, SV_OPTION_AUDIOAESROUTING, SV_AUDIOAESROUTING_0_8);
      } else {
...
void jpeg_audioaessource(sv_handle * sv, int argc, char ** argv)
{
  int value;
  int res = SV_OK;
  ...
  if(argc >= 1) {
    if(!strcmp(argv[0], "info")) {
      res = sv_option_get(sv, SV_OPTION_AUDIOAESSOURCE, &value);
      if(res == SV_OK) {
        if(value == SV_AUDIOAESSOURCE_DEFAULT) {
          printf("svram audioaessource default\n");
        } else if(value == SV_AUDIOAESSOURCE_BACK) {
          printf("svram audioaessource back\n");
        } else if(value == SV_AUDIOAESSOURCE_FRONT) {
          printf("svram audioaessource front\n");
        } else {
          printf("svram audioaessource (unknown)\n");
        }
      }
    } else if(!strcmp(argv[0], "?") || !strcmp(argv[0], "help")) {
      printf("svram audioaessource default\n");
      ...
      if(!strcmp(argv[0], "back")) {
        res = sv_option_set(sv, SV_OPTION_AUDIOAESSOURCE, SV_AUDIOAESSOURCE_BACK);
      } else if(!strcmp(argv[0], "front")) {
        res = sv_option_set(sv, SV_OPTION_AUDIOAESSOURCE, SV_AUDIOAESSOURCE_FRONT);
      } else {
        res = sv_option_set(sv, SV_OPTION_AUDIOAESSOURCE, SV_AUDIOAESSOURCE_DEFAULT);
      }
    }
  }
...
void jpeg_audionofading(sv_handle * sv, char * what)
{
  int res = SV_OK;
  int value;

  if((what[0] >= '0') && (what[0] <= '1')) {
    res = sv_option_set(sv, SV_OPTION_AUDIONOFADING, atoi(what));
  } else if(!strcmp(what, "info")) {
    res = sv_option_get(sv, SV_OPTION_AUDIONOFADING, &value);
    if(res == SV_OK) {
      printf("svram audionofading %d\n", value);
    }
  } else if(!strcmp(what, "?") || !strcmp(what, "help")) {
    printf("svram audionofading 0\t\tEnable audio fading\n");
    printf("svram audionofading 1\t\tDisable audio fading\n");
    printf("                  info\tShow current settings\n");
    return;
  } else {
    res = SV_ERROR_PARAMETER;
...
void jpeg_hdmioutput(sv_handle * sv, char * param, char * subparam)
{
  int res = SV_OK;
  int mode = SV_HDMIOUTPUT_DEFAULT;

  if(strcmp(param, "default") == 0) {
    mode = SV_HDMIOUTPUT_DEFAULT;
  } else if(strcmp(param, "3dstream") == 0) {
    if(subparam) {
      if(!strcmp(subparam, "framepacking")) {
        mode = SV_HDMIOUTPUT_3DSTREAM_FRAMEPACKING;
      } else {
        mode = SV_HDMIOUTPUT_3DSTREAM;
      }
    } else {
      mode = SV_HDMIOUTPUT_3DSTREAM;
    }
  } else if (!strcmp(param, "3dsource")) {
    if(subparam) {
      if(!strcmp(subparam, "leftright")) {
        mode = SV_HDMIOUTPUT_3DSOURCE_LEFTRIGHT;
      } else if(!strcmp(subparam, "topbottom")) {
        mode = SV_HDMIOUTPUT_3DSOURCE_TOPBOTTOM;
      } else {
        mode = SV_HDMIOUTPUT_3DSOURCE;
      }
    } else {
      mode = SV_HDMIOUTPUT_3DSOURCE;
    }
  } else if (strcmp(param, "info") == 0) {
    res = sv_option_get(sv, SV_OPTION_HDMIOUTPUT, &mode);
    if(res == SV_OK) {
      printf("sv hdmioutput %s\n", sv_option_value2string(sv, SV_OPTION_HDMIOUTPUT, mode));
    }
    return;
  } else if((strcmp(param, "?") == 0) || (strcmp(param, "help") == 0)) {
    printf("sv hdmioutput help\n");
    printf("\t3dsource default\n");
    printf("\t3dsource leftright\n");
    printf("\t3dsource topbottom\n");
    printf("\t3dstream leftright\n");
    printf("\t3dstream topbottom\n");
    return;
  } else {
    jpeg_errorprint(sv, SV_ERROR_PARAMETER);
    return;
  ...
  res = sv_option(sv, SV_OPTION_HDMIOUTPUT, mode);
...
void jpeg_debugoption(sv_handle * sv, int argc, char ** argv)
{
  int res = SV_ERROR_PARAMETER;
  
  if(argc > 1) {
    if(!strcmp(argv[1], "clear")) {
      res = sv_debugoption_clear(sv);
    } else if(!strcmp(argv[1], "set")) {
      if(argc > 2) {
        res = sv_debugoption(sv, argv[2]);
      }
    }
  }
...
void jpeg_trace(sv_handle * sv, char * pvalue)
{
  ...
  if((pvalue[0] == '-') || ((pvalue[0] >= '0') && (pvalue[0] <= '9'))) {
    ...
    if(res == SV_OK) {
      printf("sv trace %s%s%s%s%s%s%s%s%s\n", 
        (value == 0)?"0 ":"",
        ...
        (value & SV_TRACE_DIRECTAPI)?"directapi ":"");
    }
  } else if(!strcmp(pvalue, "help")) {
    printf("sv trace {#value,[error|setup|storage|fifoapi|capture|vtrcontrol|tccheck|directapi]}\n");
    return;
  } else {
    value = 0;
    ...
    if(!strstr(pvalue, "directapi")) {
      value |= SV_TRACE_DIRECTAPI; 
    }
  }
...
void jpeg_dvi(sv_handle * sv, int argc, char **argv)
{
  ...
  if(argc >= 1) {
    if(       strcmp(argv[0], "mode") == 0) {
      ...
      printf("\tmode default\tSet videomode to same as the main output pipeline\n");
...
void jpeg_monitorinfo(sv_handle * sv, int argc, char ** argv)
{
  ...
  if(res == SV_OK) {
    if(argc > arg) {
      if(!strcmp(argv[arg], "hex")) {
        printf("unsigned char monitornvram[256] = {\n");
        for(i = 0; i < 128; i+=16) {
          printf(" ");
          for(j = 0; j < 16; j++) {
  ...
  if(res == SV_OK) {
    ...
    if(buffer[0] != 0x00) crc[0] = -1;
    if(buffer[1] != 0xff) crc[0] = -1;
    if(buffer[2] != 0xff) crc[0] = -1;
    if(buffer[3] != 0xff) crc[0] = -1;
    if(buffer[4] != 0xff) crc[0] = -1;
    if(buffer[5] != 0xff) crc[0] = -1;
    if(buffer[6] != 0xff) crc[0] = -1;
    if(buffer[7] != 0x00) crc[0] = -1;
...
void jpeg_ltc(sv_handle * sv, int argc, char ** argv)
{
  ...
  if(res == SV_OK) {
    if(!strcmp(argv[0], "source") && (argc >= 2)) {
      ...
      res = sv_jack_option_get(sv, 0, SV_OPTION_ASSIGN_LTCA, &mode);
      printf("LTCA is assigned to %d\n", mode);
      return;
...
void jpeg_matrix(sv_handle * sv, int argc, char ** argv)
{
  ...
  if(argc >= 2) {
    if(!strcmp(argv[1], "info")) { 
      ...
      printf("sv %s help\n", argv[0]);
      printf("\tinfo\t\tDump matrix\n");
      printf("\tinfoex\t\tDump extended matrix\n");
      ...
      printf("\t\tDEMKey\n");
      printf("\tcustomex <filename>\t\t\tSet custom extended matrix from file\n");
      printf("\t\t#MATRIX\n");
      printf("\t\tdivisor\n");
      printf("\t\tMAT11 MAT12 MAT13\n");
      printf("\t\tMAT21 MAT22 MAT23\n");
      printf("\t\tMAT31 MAT32 MAT33\n");
      printf("\t\tMATKey\n");
      printf("\t\tMAT1_INOFFSET\n");
      printf("\t\tMAT2_INOFFSET\n");
      printf("\t\tMAT3_INOFFSET\n");
      printf("\t\tMATKey_INOFFSET\n");
      printf("\t\tMAT1_OUTOFFSET\n");
      printf("\t\tMAT2_OUTOFFSET\n");
      printf("\t\tMAT3_OUTOFFSET\n");
      printf("\t\tMATKey_OUTOFFSET\n");
      printf("\t\tDEM11 DEM12 DEM13\n");
      printf("\t\tDEM21 DEM22 DEM23\n");
      printf("\t\tDEM31 DEM32 DEM33\n");
      printf("\t\tDEMKey\n");
      printf("\t\tDEM1_INOFFSET\n");
      printf("\t\tDEM2_INOFFSET\n");
      printf("\t\tDEM3_INOFFSET\n");
      printf("\t\tDEMKey_INOFFSET\n");
      printf("\t\tDEM1_OUTOFFSET\n");
      printf("\t\tDEM2_OUTOFFSET\n");
      printf("\t\tDEM3_OUTOFFSET\n");
      printf("\t\tDEMKey_OUTOFFSET\n");
      printf("flags:\n");
...
void jpeg_anccomplete(sv_handle * sv, int argc, char ** argv)
{
  ...
  if(argc >= 2) {
    if(!strcmp(argv[1], "off") || !strcmp(argv[1], "disable")) { 
      ...
      if(res == SV_OK) {
        if((value & SV_ANCCOMPLETE_MODE_MASK) != SV_ANCCOMPLETE_OFF) {
          if((value & SV_ANCCOMPLETE_MODE_MASK) == SV_ANCCOMPLETE_STREAMER) {
            if(value & SV_ANCCOMPLETE_FLAG_FORCE_SWITCHINGLINE) {
              printf("sv anccomplete streamer forcesl\n");
            } else {
...
void jpeg_jack(sv_handle * sv, int argc, char ** argv)
{
  ...
  if((res == SV_OK) && (argc >= 3)) {
    if(!strcmp(argv[2], "assign") && (argc >= 4)) {
      if(!strcmp(argv[3], "ltc") || !strcmp(argv[3], "ltca")) {
        cmd   = SV_OPTION_ASSIGN_LTCA;
        value = 0;
        if(argc >= 5) {
          if(!strcmp(argv[4], "info")) {
            sv_jack_option_get(sv, 0, SV_OPTION_ASSIGN_LTCA, &value);
            printf("LTCA is assigned to %d\n", value);
            return;
          }
        }
...
void jpeg_hwwatchdog(sv_handle * sv,  int argc, char ** argv)
{
  ...
  if(argc >= 1) {
    if(!strcmp(argv[0], "help") || !strcmp(argv[0], "?")) {
      ...
      printf("svram hwwatchdog action none/gpi/relay/both/info\n");
      printf("svram hwwatchdog timeout #ms\n");
      printf("svram hwwatchdog timeout info\n");
      ...
      return;
    } else if(!strcmp(argv[0], "action") && (argc >= 2)) {
      if(!strcmp(argv[1], "info")) {
        res = sv_option_get(sv, SV_OPTION_HWWATCHDOG_ACTION, &value);
        if(res != SV_OK) {
          printf("sv_option_get(SV_OPTION_HWWATCHDOG_ACTION) failed %d/%s\n", res, sv_geterrortext(res));
        } else if(value == SV_HWWATCHDOG_GPI2) {
          printf("svram hwwatchdog action gpi\n");
        } else if(value == SV_HWWATCHDOG_RELAY) {
          printf("svram hwwatchdog action relay\n");
        } else if(value == (SV_HWWATCHDOG_GPI2|SV_HWWATCHDOG_RELAY)) {
          printf("svram hwwatchdog action both\n");
        } else {
          printf("svram hwwatchdog action none\n");
        }
      } else if(!strcmp(argv[1], "gpi")) {
      ...
      res = sv_option_set(sv, SV_OPTION_HWWATCHDOG_ACTION, value);
      if(res != SV_OK) {
        printf("sv_option_set(SV_OPTION_HWWATCHDOG_ACTION) failed %d/%s\n", res, sv_geterrortext(res));
      }
    } else if(!strcmp(argv[0], "timeout") && (argc >= 2)) {
      if(!strcmp(argv[1], "info")) {
        res = sv_option_get(sv, SV_OPTION_HWWATCHDOG_TIMEOUT, &value);
        if(res != SV_OK) {
          printf("sv_option_get(SV_OPTION_HWWATCHDOG_TIMEOUT) failed %d/%s\n", res, sv_geterrortext(res));
        } else {
          printf("svram hwwatchdog timeout %d\n", value);
        }
      } else {
        value = atoi(argv[1]);

        res = sv_option_set(sv, SV_OPTION_HWWATCHDOG_TIMEOUT, value);
        if(res != SV_OK) {
          printf("sv_option_set(SV_OPTION_HWWATCHDOG_TIMEOUT) failed %d/%s\n", res, sv_geterrortext(res));
        }
      }
      ...
      if(!strcmp(argv[1], "info")) {
        ...
        if(res != SV_OK) {
          ...
          printf("svram hwwatchdog relaydelay %d\n", value);
        }
      } else {
        value = atoi(argv[1]);
