...
typedef union {
  ...
  struct {
    ...
    int matrixtype;           ///< Color space (SV_MATRIXTYPE_<xxx>).
    int dataoffset;           ///< Offset to the start of the data.
  } v3;                       ///< Version 3 of the structure used for virtual
...
typedef union {
  ...
  struct {
    int xscale;               ///< X-scale factor (value is fixed point float).
    int yscale;               ///< Y-scale factor (value is fixed point float).
    int xoffset;              ///< Currently not used, set to zero (0).
    int yoffset;              ///< Currently not used, set to zero (0).
    int sharpness;            ///< Sharpness, valid from -0xffff to +0xffff, default is
                              ///< zero (0).
    int type;                 ///< Currently not used, set to zero (0).
  } v3;                       ///< Version 3 of the structure.
} sv_render_scaler;

typedef union {
  struct {
    int mode;            ///< Mode of the mixer (SV_MIXER_MODE_<xxx>).
    int position;        ///< Currently not used, set to zero (0).
    int increment;       ///< Currently not used, set to zero (0).
    int width;           ///< Currently not used, set to zero (0).
  } v1;                  ///< Version 1 of the structure.
} sv_render_wipe;
...
#define SV_RENDER_OPTION_RESILIENCE_LEVEL   2
//...

#define SV_RENDER_RESILIENCE_MASK            0x000000ff

#define SV_RENDER_RESILIENCE_BASIC           0x00000001
#define SV_RENDER_RESILIENCE_EXTENDED1       0x00000002
#define SV_RENDER_RESILIENCE_EXTENDED2       0x00000003
#define SV_RENDER_RESILIENCE_EXTENDED3       0x00000004
...
export int sv_render_realloc(sv_handle * sv, sv_render_handle * prender, sv_render_image * pimage, int version, int size, sv_render_bufferlayout * pstorage);
...
export int sv_render_push_wipe(sv_handle * sv, sv_render_handle * prender, sv_render_context * pcontext, int version, int size, sv_render_wipe * pvalue);
